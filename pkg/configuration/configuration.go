...
//go:generate $GOPATH/bin/mockgen -source=configuration.go -destination ../mocks/configuration.go -package mocks -self_package github.com/khulnasoft-lab/go-application-framework/pkg/configuration/
...
func createFileStorage(configPath string) Storage {
	file := path.Join(configPath, "vulnmap.json")
