...
func getConfig() configuration.Configuration {
	...
	config.Set(configuration.API_URL, constants.VULNMAP_DEFAULT_API_URL)
...
func Test_HttpClient_CallingNonApiUrl(t *testing.T) {
	...
	t.Run("No User-Agent", func(t *testing.T) {
		...
		assert.NotContains(t, ua, "vulnmap")
...
func Test_GetHTTPClient(t *testing.T) {
	...
	client := net.GetHttpClient()
	response, err := client.Get("https://www.khulnasoft.com")
	assert.Nil(t, err)
...
func Test_AddHeaders_AddsDefaultAndAuthHeaders(t *testing.T) {
	...
	request, _ := http.NewRequest("GET", "https://api.khulnasoft.com", nil)
...
func Test_AddUserAgent_AddsUserAgentHeaderToVulnmapApiRequests(t *testing.T) {
	app := "vulnmap-ls"
	...
	config := getConfig()
	net := NewNetworkAccess(config)
	request, err := http.NewRequest("GET", "https://api.khulnasoft.com", nil)
	assert.Nil(t, err)
...
func Test_UserAgentInfo_Complete(t *testing.T) {
	...
	t.Setenv("VULNMAP_INTEGRATION_NAME", expectedIntName)
	t.Setenv("VULNMAP_INTEGRATION_VERSION", expectedIntVersion)
	t.Setenv("VULNMAP_INTEGRATION_ENVIRONMENT", expectedIntEnvName)
	t.Setenv("VULNMAP_INTEGRATION_ENVIRONMENT_VERSION", expectedIntEnvVersion)
